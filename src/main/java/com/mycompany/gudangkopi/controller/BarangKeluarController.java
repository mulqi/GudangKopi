...
public class BarangKeluarController {
    ...
    public void tampilkanData(DefaultTableModel tableModel) {
        ...
        if (conn == null) {
            JOptionPane.showMessageDialog(null,
                    "Tidak dapat terhubung ke database. Periksa koneksi internet/jaringan Anda.",
        ...
        String sql = "SELECT ct.id AS id_transaksi, ct.transaction_date AS tanggal, "
                ...
                + "ct.party_name AS tujuan_pembeli, ct.status AS status "
                + "FROM coffee_transactions ct "
                + "JOIN coffee_grades cg ON ct.coffee_grade_id = cg.id "
                + "WHERE ct.transaction_type = 'KELUAR' "
                + "ORDER BY ct.id";
    ...
    public boolean tambahBarangKeluar(ModelBarangKeluar barang) {
        ...
        try {
            ...
            try (PreparedStatement ps = conn.prepareStatement(insertTransaksiSql)) {
                ...
                ps.setInt(2, gradeId);
                ps.setInt(3, Integer.parseInt(barang.getjumlah()));
                ps.setString(4, barang.gettujuanpembeli());
    ...
    public boolean ubahStatusBarangKeluar(int idTransaksi, String statusBaru, String JumlahBaruStr) {

        String sqlGetLama = "SELECT coffee_grade_id, quantity, status FROM coffee_transactions WHERE id = ? AND transaction_type = 'KELUAR'";
        String sqlUpdateTrans = "UPDATE coffee_transactions SET status = ?, quantity = ? WHERE id = ? AND transaction_type = 'KELUAR'";
        String sqlUpdateStok = "UPDATE coffee_grades SET stock = stock + ? WHERE id = ?"; // Tetap menggunakan '+' karena nilai kalkulasiEfekStok di bawah yang akan bernilai negatif (-)
        ...
        try {
            ...
            if (!statusLama.equalsIgnoreCase("Selesai") && statusBaru.equalsIgnoreCase("Selesai")) {
                ...
                kalkulasiEfekStok = jumlahLama - JumlahBaru; 
            }
    ...
    public boolean hapusBarangKeluar(int idTransaksi) {
        String sqlGetLama = "SELECT coffee_grade_id, quantity, status FROM coffee_transactions WHERE id = ? AND transaction_type = 'KELUAR'";
        String sqlDelete = "DELETE FROM coffee_transactions WHERE id = ? AND transaction_type = 'KELUAR'";
        ...
        try {
            ...
            conn.setAutoCommit(false); // Aktifkan transaksi database
            ...
            // 2. Jalankan perintah hapus transaksi
            try (PreparedStatement psDelete = conn.prepareStatement(sqlDelete)) {
                psDelete.setInt(1, idTransaksi);
                psDelete.executeUpdate();
            }

            // 3. Jika status lama adalah "Selesai", kembalikan jumlahnya ke stok gudang
            if (statusLama.equalsIgnoreCase("Selesai")) {
                try (PreparedStatement psStok = conn.prepareStatement(sqlUpdateStok)) {
                    psStok.setInt(1, jumlahLama); // Ditambah kembali ke stok (+jumlahLama)
                    psStok.setInt(2, gradeId);
                    psStok.executeUpdate();
                }
            }
            ...
            if (conn != null) {
                try {
                    conn.rollback(); // Batalkan semua jika terjadi error di tengah jalan
                } catch (SQLException ex) {
                    ex.printStackTrace();
                }
            }
    ...
    public void cariBarangKeluar(DefaultTableModel model, String keyword) {
